...
@property (nonatomic,strong) NSTimer * cacheTimer;//缓存时间器
...
-(NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    return _tagsTitle.count;
}
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    //标签视图
    if ([collectionView isKindOfClass:[_tagCollectionView class]]) {
    ...
    //页面视图
        ...
        if (self.params.count != 0) {
            if (![cacheController valueForKeyPath:@"XBParam"]) {
                [cacheController setValue:self.params[indexPath.item] forKeyPath:@"XBParam"];
            }
        }
...
{
    GWTagModel * model = _tagsTitle[indexPath.item];
     if ([collectionView isKindOfClass:[_tagCollectionView class]]) {     //标签
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([collectionView isKindOfClass:[_tagCollectionView class]]) {                     //选中某个标签
...
{
    if ([collectionView isKindOfClass:[_tagCollectionView class]]) {          //tag
...
- (NSTimer *)cacheTimer
{
    if (self.cacheTimer) {
        if (!_cacheTimer) {
            _cacheTimer = [NSTimer timerWithTimeInterval:5.f target:self selector:@selector(updateViewControllersCaches) userInfo:nil repeats:YES];
            [_cacheTimer setFireDate:[NSDate distantFuture]];
            [[NSRunLoop mainRunLoop] addTimer:_cacheTimer forMode:NSDefaultRunLoopMode];
        }
        return _cacheTimer;
    }
    return nil;
